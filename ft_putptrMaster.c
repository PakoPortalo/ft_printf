...
/*   Updated: 2020/09/26 19:58:26 by pako             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_flags	ft_ptrprinter(unsigned long n, t_flags data)
{
	if ((data.minus == 1) && (data.nbr < 0))
		data.width++;
	if ((data.width > data.digit) && (data.zero == 0))
		data = ft_ptrprinter1(n, data);
...
t_flags	ft_putptrMaster1(unsigned long n, t_flags data)
{
	if (data.isPrecition == 1)
		data = (data.precition <= data.digit) ? ft_ptrprinter(n, data) : ft_ptrprecition(data);
	else
		data = ft_ptrprinter(n, data);
	return (data);
}
